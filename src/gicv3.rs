...
fn get_redistributor_window_size(gicr_base: *mut GicrSgi, gic_v4: bool) -> usize {
    ...
    let first_gicr_window = unsafe {
        UniqueMmioPointer::new(
            NonNull::new(gicr_base).unwrap()
        )
    };
    ...
    if field_shared!(first_gicr, typer).read().contains(GicrTyper::VLPIS) {
        // In this case GicV4 can add 2 frames:
        // vlpi: 64KiB
        // reserved: 64KiB
